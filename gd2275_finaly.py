...
def calculate_mae_for_currency_pairs(model_name):
    ...
    for collection in collections_stats.keys():
        ...
        if not df.empty:
            ...
            mae_results[collection_name] = mae
        else:
            mae_results[collection_name] = None  # No data available for calculation
...
def create_synth_pair(cycle):
    ...
    eurchf_data = fetch_last_20_entries(collections_stats["EURCHF"], lim)
    eurcad_data = fetch_last_20_entries(collections_stats["EURCAD"], lim)
    gbpeur_data = fetch_last_20_entries(collections_stats["GBPEUR"], lim)
    gbpcad_data = fetch_last_20_entries(collections_stats["GBPCAD"], lim)
    usdchf_data = fetch_last_20_entries(collections_stats["USDCHF"], lim)
    ...
    def calculate_averages(index, *data_lists):
        keys = ['max_value', 'min_value', 'mean_value', 'vol_value', 'fd_value']
        averages = {}
        # Initialize averages dictionary
        for key in keys:
            averages['avg_' + key] = sum(data_list[index][key] for data_list in data_lists) / len(data_lists)
        return averages

    # Calculate averages and insert into new collection
    for i in range(lim):
        averages = calculate_averages(i, eurusd_data, gbpchf_data, eurchf_data, eurcad_data, gbpeur_data, gbpcad_data, usdchf_data, usdcad_data)
        print(f"Averages for index {i}: {averages}")

        # Inserting aggregated data into the new collection
        synth_collection.insert_one({
            "max_value": averages['avg_max_value'],
            "min_value": averages['avg_min_value'],
            "mean_value": averages['avg_mean_value'],
            "vol_value": averages['avg_vol_value'],
            "fd_value": averages['avg_fd_value']
        })
...
def trade(long_price_initial, short_price_initial, long_price_final, short_price_final, investment=100):
    print("L/S ratios:")
    print(long_price_initial/short_price_initial)
    print(long_price_final/short_price_final)
               
    units_long = investment / long_price_initial
    units_short = investment / short_price_initial

    # Calculate P/L for long and short positions
    pl_long = (long_price_final - long_price_initial) * units_long
    pl_short = (short_price_initial - short_price_final) * units_short
...
def main():
    ...
    for i in range(1, 81):  
        ...
        if i in [50, 60, 70]:
            ...
            print(f"Running Trade no {i/10-4}")

            gbpusd_p2_data = fetch_last_20_entries(collections_stats["GBPUSD"], 20)
            usdjpy_p2_data = fetch_last_20_entries(collections_stats["USDJPY"], 20)

            # Creating DataFrames for each currency pair
            mean_values_usdjpy = [(8*entry['mean_value']) for entry in usdjpy_p2_data]
            fx_timestamp = [entry['fx_timestamp'] for entry in usdjpy_p2_data]
            df_usdjpy = pd.DataFrame(data={'timestamp': fx_timestamp, 'price': mean_values_usdjpy})
            
            mean_values_gbpusd = [(entry['mean_value']) for entry in gbpusd_p2_data]
            fx_timestamp = [entry['fx_timestamp'] for entry in gbpusd_p2_data]
            df_gbpusd = pd.DataFrame(data={'timestamp': fx_timestamp, 'price': mean_values_gbpusd})

            # Get slopes from predicted prices
            slope_usdjpy = calculate_slope(df_usdjpy['price'])
            print("slope_usdjpy: "+ str(slope_usdjpy))
            slope_gbpusd = calculate_slope(df_gbpusd['price'])
            print("slope_gbpusd: "+ str(slope_gbpusd))

            # Determine which currency pair to go long and which to go short
            long_cp, short_cp = ('USDJPY', 'GBPUSD') if slope_usdjpy > slope_gbpusd else ('GBPUSD', 'USDJPY')
            print("long_cp: "+ str(long_cp))
            print("short_cp: "+ str(short_cp))

        if i in [60, 70, 80]:
            gbpusd_data = fetch_last_20_entries(collections_stats["GBPUSD"], 10)
            usdjpy_data = fetch_last_20_entries(collections_stats["USDJPY"], 10)

            long_data, short_data = (usdjpy_data['price'].tolist(), gbpusd_data['price'].tolist())
            # Perform trading at hour 5, 6, 7 and close at 8
            investment_per_trade = 100
            pl_results = []
            
            pl = trade(long_data[-1], short_data[-1], long_data[0], short_data[0], investment_per_trade)
            pl_results.append(pl)

            # Summing all P/Ls
            total_pl = sum(pl_results)
            print("Total Profit/Loss over the strategy period:", total_pl)
